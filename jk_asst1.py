...
def problem1(numh = 1000, x=1): 
    hs = np.logspace(-16,0,num=numh)
    errs = np.abs(np.exp(x)-(-np.exp(x+2*hs)+8*np.exp(x+hs)+np.exp(x-2*hs)-8*np.exp(x-hs))/(12*hs))
    plt.plot(np.log10(hs), np.log10(errs), label='exp(x)')
