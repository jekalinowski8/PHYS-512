...
def cal_energy(x,m,epsilon=0.5):
     n = len(x)
...
def cal_force_mesh(r,m,grid=100,eps=0.05,periodic=False):  
    ...
    xs = np.linspace(-grid//2,grid//2,num=bgrid+1)[:-1]
    r2 = np.tile(xs**2,(bgrid,1))
    r = np.sqrt(r2+r2.transpose()+eps**2) 
    pot = -G/r
    #phi = fftconvolve(pot,density,mode='full')
    phi = np.real(np.fft.ifft2(np.fft.fft2(density)*np.fft.fft2(pot),s=(grid,grid)))
    phi = np.fft.fftshift(phi)
   # plt.imshow(phi)
    xgrad,ygrad = np.gradient(-phi)
   # plt.figure(3); plt.imshow(phi)
   #assert(1==0
